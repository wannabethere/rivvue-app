 ...
 */
public class HystrixInitialization {

	public static void main(String args[]) {
		new HystrixInitialization().init();
	}

	public HystrixInitialization() {
		/*
		...
		ConfigurationManager.getConfigInstance().setProperty("hystrix.command.LoginUpdateFBFriendsCommand.execution.isolation.thread.timeoutInMilliseconds", 5000);
		...
		Thread t = new Thread(new Runnable() {
			...
			public void run() {
				while (true) {
					...
					HystrixCommandMetrics creditCardMetrics = HystrixCommandMetrics.getInstance(HystrixCommandKey.Factory.asKey(LoginUpdateFBFriendsCommand.class.getSimpleName()));
