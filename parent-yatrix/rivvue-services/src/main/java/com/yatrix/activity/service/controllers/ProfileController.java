...
public class ProfileController {
  ...
  private UserAccountRepository userRepository;
	...
	@RequestMapping(
	  ...
	  public @ResponseBody
	  UserProfile create(@PathVariable String userId, @RequestParam String username, @RequestParam String firstName,
  ...
  @RequestMapping(
	  ...
	  UserProfile createContact(@PathVariable String refUserId, @RequestParam String userId, @RequestParam String username, @RequestParam String firstName,
		...
		Reference pSignificantOther = new Reference(REFERENCETYPE.CONTACTS,
				refUserId, null, null, null);
		profile.setSignificantOther(pSignificantOther);
...
@RequestMapping(
	  ...
	  if(dto != null) {
		...
		UserAccount createdBy = userRepository.findByUserId(refUserId);
		createdBy.getAppFriends().add(user.getUsername());
		
		userRepository.save(createdBy);
		
		List<UserAccount> appFriends = userRepository.findByUsernameIn(createdBy.getAppFriends().toArray(new String[0]));
  ...
  ProfileListDto searchUsers(@PathVariable String userId, @RequestParam String query) {
	...
	public String search(@PathVariable String username, ModelMap model, @RequestParam String query) {
		...
		List<UserAccount> userAccounts = userRepository.findByEmailOrFirstNameOrLastNameLike(query);
	...
	String addFriend(@PathVariable String userId,
		...
		if(UserProfile.PROFILETYPE.FB.toString().equalsIgnoreCase(profileType)){
			if(!user.getFacebookFriends().contains(id)){
				user.getFacebookFriends().add(id);
				userRepository.save(user);
				return "Friend added successfully";
			}
		} else if(UserProfile.PROFILETYPE.APP.toString().equalsIgnoreCase(profileType)){

			if(!user.getAppFriends().contains(id)){
				user.getAppFriends().add(id);
				userRepository.save(user);
				return "Friend added successfully";
			}
