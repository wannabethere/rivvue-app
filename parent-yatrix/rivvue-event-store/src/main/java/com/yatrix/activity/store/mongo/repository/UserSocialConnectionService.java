 ...
 */
@Service
public class UserSocialConnectionService
  implements ConnectionService
{
  ...
  @Autowired
  public UserSocialConnectionService(MongoTemplate mongoTemplate, ConnectionConverter converter) {
  ...
  public int getMaxRank(String userId, String providerId) {
    ...
    // = ?
    String usersId = "";
    // if(!userId.startsWith(UserAdminService.USER_ID_PREFIX)){
    // usersId=UserAdminService.USER_ID_PREFIX+userId;
    // }
    // else{
    usersId = userId;
    // }
    Query q = query(where("userId").is(usersId).and("providerId").is(providerId));
    q.sort().on("rank", Order.DESCENDING);
    UserSocialConnection cnn = mongoTemplate.findOne(q, UserSocialConnection.class);
  ...
  public void remove(String userId, ConnectionKey connectionKey) {
    ...
    usersId = userId;
    // }
    Query q = query(where("userId").is(usersId).and("providerId").is(connectionKey.getProviderId())
      .and("providerUserId").is(connectionKey.getProviderUserId()));
    mongoTemplate.remove(q, UserSocialConnection.class);
  }
  ...
  public Connection<?> getPrimaryConnection(String userId, String providerId) {
    ...
    Query q = query(where("userId").is(usersId).and("providerId").is(providerId).and("rank").is(1));
  ...
  public List<Connection<?>> getConnections(String userId) {
    ...
    usersId = userId;
    // }
    Query q = query(where("userId").is(usersId));
  ...
  public List<Connection<?>> getConnections(String userId, String providerId) {
    ...
    usersId = userId;
    // }
    Query q = new Query(where("userId").is(usersId).and("providerId").is(providerId));
  ...
  private List<Connection<?>> runQuery(Query query) {
    List<UserSocialConnection> results = mongoTemplate.find(query, UserSocialConnection.class);
